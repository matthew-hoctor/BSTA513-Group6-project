...
MrOs <- MrOs[c(-PO7440, -PI5413, -BI0650, -PA3753)] # not how you censor individual observations.
glimpse(MrOs)
skimr::skim(MrOs)
obj_classes <- c()
...
df_fullmod <- data.frame(summary(full_model)$coef)
df_fullmod$test <- summary(full_model)$coef[,4] < 0.05 
df_fullmod
```
...
# exclude hx. diabetes,
reduced_model <-  
  glm(falls ~  giage1 + mhstrk + mhpark + mhcopd + mharth + mhcancer +
...
# exclude hx stroke
reduced_model <-  
  glm(falls ~  giage1 + mhdiab + mhpark + mhcopd + mharth + mhcancer +
        pascore + qlh + hwbmi + gsgrpavg + nfwlkspd + b1fnd + b1tbfkg + st_1 +
        st_2 + st_3 + st_4 + st_5,  
      family = "binomial",
      data = step2_narm)
...
# exclude hx cancer
reduced_model <-  
  glm(falls ~  giage1 + mhdiab + mhstrk + mhpark + mhcopd + mharth + pascore +
        qlh + hwbmi + gsgrpavg + nfwlkspd + b1fnd + b1tbfkg + st_1 +
        st_2 + st_3 + st_4 + st_5,  
      family = "binomial",
      data = step2_narm)
...
### step 6: preliminary final model
...
The small p-value reported above suggests that the model does not fit well; however out sample size is quite large (> 1000), and therefore the Hosmer-Lemeshow test is likely to reject the model even if the fit is adequate.
...
Four influencial outliers are identified.  Consultation with a subject matter expert would allow us to make an informed decision on whether to include or exclude these subjects.  It appears that these four outliers represent the extreme values of the inverse square of the walking speed.  We can censor PO7440, PI5413, BI0650, & PA3753;  and repeat the analysis.  A separate .rmd file will be made for this.
